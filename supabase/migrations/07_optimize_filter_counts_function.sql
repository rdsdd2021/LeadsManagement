-- Optimized filter counts function - uses single query with CTEs instead of multiple table scans
-- This should be 10-50x faster than the previous version
...
BEGIN
  ...
  WITH filtered_base AS (
    SELECT school, district, gender, stream
    FROM leads
    WHERE TRUE
...
COMMENT ON FUNCTION get_filter_counts IS 'Optimized filter counts using single CTE - filters schools with <20 leads';
